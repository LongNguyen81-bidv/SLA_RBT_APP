...
function SLAStepRowItem({step, external}) {
    return (<div className="px-4 py-3 border-b border-[#E8F5F3] flex justify-between items-center bg-white hover:bg-surface-50 transition-colors">
        <div>
            ...
            <div className="text-[10px] text-[#94B5B0] font-sans"> {
                step.owner
            }</div>
        </div>
        <div className="text-right">
            ...
            <div className={
                `text-[9px] font-mono mt-0.5 ${
                    external ? 'text-[#94B5B0]' : step.system === 'RLOS' ? 'text-bidv-green-mid' : 'text-[#94B5B0]'
                }`
            }> {
...
export default function SLAConfigPanel({SLA_STEPS, TOTAL_INTERNAL_HOURS}) {
    ...
    return (<div className="grid grid-cols-1 lg:grid-cols-2 gap-5"> {/* Internal */}
        <div>
            ...
            <div className="bg-white border border-[#C5DED9] rounded-[10px] overflow-hidden shadow-sm">
                <div className="px-4 py-2.5 border-b border-bidv-green-tint flex justify-between text-[11px] text-[#6B9E97] bg-bidv-green-surface font-sans">
                    ...
                    <span className="font-mono text-bidv-green font-bold"> {TOTAL_INTERNAL_HOURS}h
                    </span>
                </div>
                {
                internalSteps.map((step) => (<SLAStepRowItem key={
                        step.id
                    }
                    step={step}
                    external={false}/>))
        ...
        <div>
            ...
            <div className="bg-white border border-[#C5DED9] rounded-[10px] overflow-hidden mb-4 shadow-sm"> {
                externalSteps.map((step) => (<SLAStepRowItem key={
                        step.id
                    }
                    step={step}
                    external={true}/>))
