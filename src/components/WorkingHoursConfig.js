...
export default function WorkingHoursConfig() {
    ...
    const toggleDay = (day) => {
        setConfig((prev) => {
            const days = prev.workDays.includes(day) ? prev.workDays.filter((d) => d !== day) : [
                ...prev.workDays,
                day
            ].sort((a, b) => a - b);
            return {
    ...
    return (<div className="bg-white border border-[#C5DED9] rounded-[10px] overflow-hidden shadow-sm"> {/* Header */}
        ...
        <div className="p-4 space-y-5"> {/* Work hours */}
            <div>
            ...
            <div>
                ...
                {
                config.lunchBreakEnabled && (<div className="flex items-center gap-3 animate-fadeIn">
                    <input type="time"
                        value={
                            hourToTime(config.lunchBreak.start)
                        }
                        onChange={
                            (e) => setConfig((prev) => ({
                                ...prev,
                    ...
                    <input type="time"
                        value={
                            hourToTime(config.lunchBreak.end)
                        }
                        onChange={
                            (e) => setConfig((prev) => ({
                                ...prev,
                ...
                <div className="flex gap-1.5"> {
                    DAY_LABELS.map(({value, label}) => {
                        ...
                        return (<button key={value}
                            ...
                            className={
                                `w-10 h-10 rounded-lg text-xs font-semibold transition-all duration-200 border ${
                                    active ? 'bg-bidv-green text-white border-bidv-green shadow-sm' : 'bg-surface-50 text-[#94B5B0] border-[#C5DED9] hover:border-bidv-green-mid hover:text-bidv-green-mid'
                                }`
                        }> {label} </button>);
                    })
                } </div>
            ...
            <div className="bg-[#F0FAF8] rounded-lg p-3 border border-[#D5EBE7]">
                ...
                <div className="text-xs text-[#1a3329] font-sans leading-relaxed">
                    ...
                    <span className="font-semibold"> {
                        config.endHour - config.startHour -(config.lunchBreakEnabled ? config.lunchBreak.end - config.lunchBreak.start : 0)
                    }h
                    </span>
                    /ngày
                </div>
