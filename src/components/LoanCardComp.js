...
export default function LoanCardComp({loan, progress, onClick, selected}) {
    ...
    const hasExceeded = progress.some((p, i) => p.actualHours && getSLAStatus(p.actualHours, SLA_STEPS[i].slaHours) === 'exceeded');
    const hasWarning = progress.some((p, i) => p.actualHours && getSLAStatus(p.actualHours, SLA_STEPS[i].slaHours) === 'warning');
    ...
    return (<div onClick={onClick}
        ...
        <div className="flex gap-0.5 mb-2"> {
            SLA_STEPS.map((step, i) => {
                const s = progress[i];
                const st = s.completed ? getSLAStatus(s.actualHours, step.slaHours) : i === currentStep ? 'warning' : 'pending';
                const colorMap = {
                    ok: 'bg-emerald-500',
