...
export default function LoanDetailPanel({loan, progress, SLA_STEPS, TOTAL_INTERNAL_HOURS}) {
    ...
    const totalElapsed = getElapsedHours(loan.startTime);
    ...
    const handleAction = (stepId, actionType) => {
        if (window.confirm(`Xác nhận ${
            actionType === 'FORWARD' ? 'Hoàn thành và Bàn giao' : 'Trả lại'
        } bộ hồ sơ này?`)) {
    ...
    return (<div> {/* Loan header info */}
        <div className="mb-4">
            <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                    ...
                    <div className={
                        `text-[13px] font-semibold font-sans mt-0.5 ${
                            loan.currentStepId ? 'text-orange-600' : 'text-green-600'
                        }`
                    }> {
                        loan.currentStepId ? `Đang xử lý tại ${
                            loan.assignedDept
                        }` : 'Hoàn thành toàn bộ'
                    } </div>
                </div>
        ...
        <div className="flex flex-col gap-1.5 mb-5 relative"> {
            mutation.isPending && (<div className="absolute inset-0 bg-white/50 z-10 flex items-center justify-center rounded-lg">
            ...
            SLA_STEPS.filter((s) => s.internal).map((step) => {
                ...
                const hasPermission = isActive && (user ?. role === 'ADMIN' || user ?. dept === loan.assignedDept);
                ...
                return (<div key={
                    ...
                    <StepCard step={step}
                        progress={prog}
                        isActive={isActive}/> {/* Handover Actions */}
                    {
                    hasPermission && (<div className="absolute top-1/2 -translate-y-1/2 right-4 flex gap-2"> {
                        step.id > 1 && (<button onClick={
                                () => handleAction(step.id, 'BACKWARD')
                            }
