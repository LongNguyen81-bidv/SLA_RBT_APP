...
export default function StepCard({step, progress, isActive}) { // Subscribe to config changes so business-hours-aware elapsed time recalculates
    const {config} = useConfig();

    const currentActualHours = (isActive && progress.startedAt) ? getElapsedHours(progress.startedAt) : progress.actualHours;
    const status = progress.completed ? getSLAStatus(progress.actualHours, step.slaHours) : (isActive && progress.startedAt) ? getSLAStatus(currentActualHours, step.slaHours) : 'pending';
    ...
    return (<div className={
        ...
        {
        isActive && progress.startedAt && (<div className="mt-2 text-[10px] text-[#94B5B0] font-mono flex items-center gap-1">
            <span>⏱</span>
            <span>Tính theo giờ làm việc ({workHoursPerDay}h/ngày, {
                config.workDays.length
            }
                ngày/tuần)</span>
        </div>)
