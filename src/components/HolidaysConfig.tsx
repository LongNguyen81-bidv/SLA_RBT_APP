...
export default function HolidaysConfig() {
  ...
  const holidays: Holiday[] = config.holidays || [];
  ...
  const grouped: Record<string, Holiday[]> = holidays.reduce(
    (acc: Record<string, Holiday[]>, h) => {
        ...
        <div className="flex gap-2 items-end flex-wrap">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all duration-200 shadow-sm ${
              newDate
                ? 'bg-bidv-green text-white hover:bg-bidv-green/90 active:scale-95'
