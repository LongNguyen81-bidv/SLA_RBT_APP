...
export default function SLAStepsTable({SLA_STEPS, allProgress}) { // Subscribe to config changes so realtime hours recalculate
    ...
    return (<div>
        <div className="flex items-center justify-between mb-3">
            <div className="text-[13px] font-semibold text-[#6B9E97] font-mono">
                TỔNG HỢP THEO BƯỚC
            </div>
            <div className="text-[10px] text-[#94B5B0] font-mono flex items-center gap-1.5">
                <span>⏱</span>
                <span>Giờ làm việc: {workHoursPerDay}h/ngày · {
                ...
                <thead>
                    <tr className="border-b border-bidv-green-tint bg-bidv-green-surface"> {
                        [
                            ...
                            className="px-3.5 py-2.5 text-left text-[10px] text-[#6B9E97] font-mono font-semibold tracking-wider"> {h} </th>))
                    } </tr>
                </thead>
                <tbody> {
                    SLA_STEPS.map((step, si) => {
                        const statuses = allProgress.map((prog) => {
                            ...
                            if (p.completed && p.actualHours != null) {
                                ...
                                const currentHours = getElapsedHours(p.startedAt);
                        ...
                        return (<tr key={
                            ...
                            <td className="px-3.5 py-2.5">
                                <span className="font-mono text-[11px] text-[#6B9E97]"> {
                                    step.code
                                } </span>
                            </td>
                            <td className="px-3.5 py-2.5 text-xs text-[#1a3329] font-sans"> {
                                ...
                                {
                                !step.internal && (<span className="ml-1.5 text-[9px] text-bidv-gold font-mono inline-block">
                                    NGOÀI NH
                                </span>)
                            } </td>
                            ...
                            <td className="px-3.5 py-2.5"> {
                                ok > 0 && (<span className="text-xs text-emerald-800 font-mono font-semibold"> {ok} </span>)
                            } </td>
                            <td className="px-3.5 py-2.5"> {
                                warn > 0 && (<span className="text-xs text-amber-800 font-mono font-semibold"> {warn} </span>)
                            } </td>
                            <td className="px-3.5 py-2.5"> {
                                exc > 0 && (<span className="text-xs text-red-800 font-mono font-bold"> {exc}⚠</span>)
                            } </td>
                        </tr>);
                    })
                } </tbody>
