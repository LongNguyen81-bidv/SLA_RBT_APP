...
let currentLoans = [...MOCK_LOANS];
let currentProgress = JSON.parse(JSON.stringify(MOCK_PROGRESS));
...
export const loansApi = {
  getLoans: async (currentUser) => {
    if (USE_MOCK) {
      await delay(800);

      let filteredLoans = currentLoans;
      let filteredProgress = currentProgress;

      // Phân quyền dữ liệu trả về
      if (currentUser?.role !== 'ADMIN') {
        const dept = currentUser?.dept;
        filteredLoans = currentLoans.filter((loan, index) => {
  ...
  completeStep: async (loanId, stepId, actionType) => {
    if (USE_MOCK) {
      await delay(600);
