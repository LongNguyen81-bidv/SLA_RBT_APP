...
import type {StaffPerf as StaffPerfType, SLAStatus}
from '../types';
            ...
            <div className="bg-white border border-[#C5DED9] rounded-[10px] shadow-sm overflow-x-auto">
                <table className="w-full border-collapse min-w-[700px]">
                    ...
                    <tbody> {' '}
                        {
                        STAFF_PERF.map((s, i) => {
                            const hasData = s.loans > 0;
                            const perf = ! hasData ? '—' : s.exceeded === 0 ? 'Xuất sắc' : s.exceeded <= 1 ? 'Tốt' : s.exceeded <= 2 ? 'Trung bình' : 'Cần cải thiện';
                            const badgeStatus: SLAStatus = ! hasData ? 'pending' : s.exceeded === 0 ? 'ok' : s.exceeded <= 1 ? 'ok' : s.exceeded <= 2 ? 'warning' : 'exceeded';
                            return (
                                <tr key={i}
                                    className="border-b border-[#E8F5F3] hover:bg-surface-50 transition-colors">
                                    <td className="px-4 py-3">
                                        <div className="text-[12px] font-mono font-semibold text-[#1a3329]">
                                            {' '}
                                            {
                                            s.staffCode
                                        }
                                    ...
                                    <td className="px-4 py-3 text-[11px] font-mono text-[#94B5B0]">
                                        {
                                        s.deptCode
                                    } </td>
                                    <td className="px-4 py-3 font-mono text-xs text-bidv-green font-semibold">
                                        {' '}
                                        {
                                        hasData ? formatNumber(s.loans) : '—'
                                    }
                                        {' '} </td>
                                    <td className="px-4 py-3">
                                        <span className={
                                            `font-mono text-xs ${
                                                ! hasData ? 'text-[#c0c0c0]' : s.avgHours > 3 ? 'text-amber-800 font-semibold' : 'text-[#6B9E97] font-normal'
                                            }`
                                        }>
                                            {' '}
                                            {
                                    ...
                                    <td className="px-4 py-3">
                                        ...
                                        {
                                        ! hasData ? (
                                            <span className="text-xs text-[#c0c0c0] font-mono">—</span>
                                        ) : s.exceeded > 0 ? (
                                    ...
                                    <td className="px-4 py-3">
                                        ...
                                        hasData ? (
                                            ...
                                            <span className="text-xs text-[#c0c0c0] font-sans">—</span>
                                        )
                                    } </td>
                                </tr>
                            );
