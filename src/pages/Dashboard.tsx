    ...
    <div>
        ...
        <div className="flex items-center gap-2 text-[11px] text-[#6B9E97] font-mono">
          ...
          <span>
            ...
            <span className="font-semibold text-bidv-green">
              {' '}
              {String(Math.floor(config.startHour)).padStart(2, '0')}:00 →{' '}
              {String(Math.floor(config.endHour)).padStart(2, '0')}:00
            </span>
            {config.lunchBreakEnabled && (
              <>
                ...
                <span className="font-semibold text-bidv-gold-dark">
                  {' '}
                  {Math.floor(config.lunchBreak.start)}:00 → {Math.floor(config.lunchBreak.end)}:00
                </span>
              </>
            )}
            {' · '}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-7">
        <MetricCard
          ...
          value={totalActive}
          sub={`trong tổng số ${loans.length} hồ sơ`}
          accent="#004D40"
        />
        <MetricCard
          label="Bước vượt SLA"
